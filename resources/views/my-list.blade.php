...
@section('content')
    ...
    <div class="workersForm">
        @foreach ($dataSet as $data)
            ...
            @if ($type !== 'Bedrijven' && !$data->admin)
                <form class="workerForm" 
                      action="{{ $type === 'Personeel' ? route('getData') : route('specials') }}" 
                      method="post">
                    @csrf
            ...
            @elseif ($type === 'Bedrijven')
                <form class="workerForm" 
                      ...
                      method="get">
                    @csrf <!-- Niet nodig voor GET, kan weg -->
                    <button class="workerButton" type="submit" name="worker" style="display: flex; align-items: center; justify-content: center;">
                        {{ $data->company_name }}
        ...
        @if ($type === 'Voor wie?')
            <form class="workerForm" method="post" action="{{ route('specials') }}">
                @csrf
                <button class="workerButton" type="submit" name="worker" value="{{ $dataSet }}">Voor iedereen</button>
            </form>
        @endif
    </div>
    ...
    @if ($type === 'Personeel')
        <a href="{{ route('myList', ['type' => 'Voor wie?', 'company_code' => auth()->user()->company_code]) }}"
